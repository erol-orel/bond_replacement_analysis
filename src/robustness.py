...
def _metrics(r, rf=None):
    ...
    sharpe = (ex.mean() * PER) / vol if vol > 0 else np.nan
    peak = np.maximum.accumulate(lvl)
    dd = (lvl / peak - 1).min()
    var95 = np.percentile(r, 5)
    cvar95 = r[r <= var95].mean() if np.any(r <= var95) else var95
    return cagr, sharpe, dd, cvar95
...
def bootstrap_ci(px):
    ...
    band-rebalanced strategies: ΔCAGR/ΔSharpe/ΔMaxDD/ΔCVaR95 (replacement − AP5). Sharpe is
    ...
    for j, name in enumerate(cols):
        if name == "AP5":
            continue
        dC, dS, dD, dV = (np.empty(N_BOOT) for _ in range(4))
        for i in range(N_BOOT):
            rr = arr[idx[i]]; rf = cash[idx[i]]
            cj, sj, dj, vj = _metrics(rr[:, j], rf)
            cb, sb, db, vb = _metrics(rr[:, base], rf)
            dC[i], dS[i], dD[i], dV[i] = cj - cb, sj - sb, dj - db, vj - vb
        rows[name] = dict(
            ...
            dCVaR_med=np.median(dV), dCVaR_p5=np.percentile(dV, 5), dCVaR_p95=np.percentile(dV, 95),
            P_worse_cvar=float(np.mean(dV < 0)))
...
def main():
    ...
    a2.plot(x, ci["P_worse_cvar"] * 100, "s--", color="tab:orange", label="P(worse CVaR95)")
    ...
    show = ["dSharpe_med", "dSharpe_p5", "dSharpe_p95", "P_dSharpe_gt0",
            "dMaxDD_p5", "dMaxDD_p95", "P_worse_drawdown", "P_worse_cvar"]
    print((ci[show] * [1, 1, 1, 1, 100, 100, 1, 1]).round(3).to_string())
